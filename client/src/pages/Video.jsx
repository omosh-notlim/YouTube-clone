...
const Video = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // await axios.patch(`/videos/view/${path}`);
  ...
  const handleLike = async () => {
    await axios.patch(`/users/like/${currentVideo.id}`);
    dispatch(like(currentUser.id));
  };
  const handleDislike = async () => {
    await axios.patch(`/users/dislike/${currentVideo.id}`);
    dispatch(dislike(currentUser.id));
  };

  const handleSub = async () => {
    currentUser.subscribedUsers.includes(channel.id)
      ? await axios.patch(`/users/unsub/${channel.id}`)
      : await axios.patch(`/users/sub/${channel.id}`);
  ...
  return (
    <Container>
      <Content>
        ...
        <Details>
          <Info>{currentVideo?.views} views • {currentVideo?.createdAt && format(currentVideo.createdAt)}</Info>
          <Buttons>
